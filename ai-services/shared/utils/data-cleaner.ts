...
export class DataCleaner {
   ...
   * 清洗健康数据
   */
  cleanHealthData(data: HealthData): HealthData {
    try {
      return {
        ...data,
   ...
   * 清洗物理数据
   */
  private cleanPhysicalData(data: HealthData['physicalData']): HealthData['physicalData'] {
    return {
      ...data,
      // 移除异常值
   ...
   * 清洗心理数据
   */
  private cleanMentalData(data: HealthData['mentalData']): HealthData['mentalData'] {
    return {
      ...data,
      // 标准化评分
   ...
   * 清洗营养数据
   */
  private cleanNutritionData(data: HealthData['nutritionData']): HealthData['nutritionData'] {
    return {
      ...data,
      // 移除异常值
   ...
   * 清洗生活方式数据
   */
  private cleanLifestyleData(data: HealthData['lifestyleData']): HealthData['lifestyleData'] {
    return {
      ...data,
      // 移除异常值
   ...
   * 清洗餐食数据
   */
  private cleanMeals(meals: HealthData['nutritionData']['meals']): HealthData['nutritionData']['meals'] {
    return meals
      // 移除无效餐食
      .filter(meal => 
        meal.type && 
   ...
   * 清洗活动数据
   */
  private cleanActivities(activities: HealthData['lifestyleData']['activities']): HealthData['lifestyleData']['activities'] {
    return activities
      // 移除无效活动
      .filter(activity => 
        activity.type && 
