...
describe('AuditLogService', () => {
  ...
  describe('query', () => {
    beforeEach(async () => {
      // 准备测试数据
      const testLogs = Array(20).fill(null).map((_, index) => ({
        userId: `user-${index % 2}`,
  ...
  describe('getStats', () => {
    beforeEach(async () => {
      // 准备测试数据
      const testLogs = Array(50).fill(null).map((_, index) => ({
        userId: `user-${index % 5}`,
  ...
  describe('export', () => {
    beforeEach(async () => {
      // 准备测试数据
      const testLogs = Array(10).fill(null).map((_, index) => ({
        userId: `user-${index}`,
        event: 'login',
        category: 'authentication',
        action: 'user login',
  ...
  describe('清理过期日志', () => {
    it('应该能够清理过期日志', async () => {
      // 准备测试数据
      const oldLogs = Array(5).fill(null).map((_, index) => ({
        userId: `user-${index}`,
        event: 'login',
        category: 'authentication',
        action: 'user login',
        status: 'success',
        timestamp: new Date(Date.now() - (SecurityConfig.AUDIT_LOG.retention + 24 * 60 * 60 * 1000)) // 比保留期限多一天
      }));

      const newLogs = Array(5).fill(null).map((_, index) => ({
        userId: `user-${index}`,
        event: 'login',
        category: 'authentication',
        action: 'user login',
