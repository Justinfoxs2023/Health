 ...
 * 训练所有模型
 */
export async function trainAllModels(trainingData: IHealthData[]): Promise<void> {
  try {
...
async function trainVitalSignsModel(
  trainData: IHealthData[],
  testData: IHealthData[],
): Promise<void> {
  try {
 ...
 * 划分训练集和测试集
 */
function splitData(data: IHealthData[]): {
  trainData: IHealthData[];
  testData: IHealthData[];
} {
  // 随机打乱数据
 ...
 * 检查模型性能
 */
function checkModelPerformance(evaluation: { loss: number; accuracy: number }): boolean {
  return (
