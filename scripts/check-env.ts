...
async function checkEnvironmentConfig() {
  ...
  try {
    ...
    if (storageConfig.driver === 's3') {
      checkObjectValues(storageConfig.aws, [
        'region',
        'accessKeyId',
        'secretAccessKey',
        'bucket'
...
function checkConfigGroup(configService: ConfigService, keys: string[]) {
  keys.forEach(key => {
...
function checkObjectValues(obj: any, requiredKeys: string[]) {
...
// 检查环境变量值的有效性
function validateEnvironmentValues(configService: ConfigService) {
  const validations = [
    {
      key: 'APP_PORT',
...
// 检查URL格式
function validateUrls(configService: ConfigService) {
  const urlConfigs = [
    'APP_URL',
    'AI_SERVICE_URL',
    'AWS_S3_ENDPOINT',
...
// 检查敏感配置的加密
function checkSensitiveConfigs(configService: ConfigService) {
  const sensitiveKeys = [
    'DB_PASS',
    'REDIS_PASSWORD',
    'JWT_SECRET',
