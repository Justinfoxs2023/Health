...
export const FeedbackScreen = ({ navigation }) => {
  ...
  const feedbackTypes: IFeedbackType[] = [
    {
      key: 'bug',
      title: '功能异常',
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.description}>您的反馈对我们很重要，我们会认真处理每一条反馈信息</Text>
      </View>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.typeGrid}>
          {feedbackTypes.map(type => (
            <TouchableOpacity
              key={type.key}
              style={[styles.typeButton, form.type === type.key && styles.typeButtonActive]}
              onPress={() => setForm(prev => ({ ...prev, type: type.key }))}
            >
              <Text style={[styles.typeTitle, form.type === type.key && styles.typeTitleActive]}>
                {type.title}
              </Text>
              <Text style={[styles.typeDesc, form.type === type.key && styles.typeDescActive]}>
                {type.description}
              </Text>
            </TouchableOpacity>
          ))}
