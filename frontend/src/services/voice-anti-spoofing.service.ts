...
export class VoiceAntiSpoofingService {
  ...
  private async initializeProcessor() {
    try {
      ...
      this.realtimeProcessor = new AudioWorkletNode(
        this.audioContext,
        'anti-spoofing-processor'
      );
    } catch (error) {
      console.error('初始化实时处理器失败:', error);
    }
  }
  ...
  // 更新防伪配置
  async updateConfig(newConfig: Partial<AntiSpoofingConfig>): Promise<void> {
    this.config = {
      ...this.config,
      ...newConfig
    };
  ...
  async getStatistics(): Promise<any> {
    return await this.db.get('anti-spoofing-stats') || {
      totalChecks: 0,
      attacksDetected: 0,
      attackTypes: {}
    };
