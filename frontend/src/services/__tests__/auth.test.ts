...
describe('JWTService', () => {
  ...
  describe('generateTokens', () => {
    ...
    it('应该在生成令牌时记录性能指标', async () => {
      ...
      expect(monitoringService.recordRequestPerformance).toHaveBeenCalledWith(
        expect.any(Number),
        expect.any(Number),
      );

      const [recordedStart, recordedEnd] = (monitoringService.recordRequestPerformance as jest.Mock)
        .mock.calls[0];
  ...
  describe('refreshAccessToken', () => {
    ...
    it('应该拒绝无效的刷新令牌', async () => {
      await expect(JWTService.refreshAccessToken('invalid-refresh-token')).rejects.toThrow();
  ...
  describe('revokeRefreshToken', () => {
    it('应该成功撤销刷新令牌', async () => {
      ...
      // 验证令牌已被撤销
      await expect(JWTService.verifyRefreshToken(mockTokens.refreshToken)).rejects.toThrow(
        '刷新令牌已失效',
      );
    });
  });
  ...
  describe('错误���理', () => {
    it('应该正确处理数据库错误', async () => {
      ...
      await expect(JWTService.generateTokens(mockUserId)).rejects.toThrow();

      expect(monitoringService.recordError).toHaveBeenCalledWith(dbError, expect.any(Object));
    });
    ...
    it('应该正确处理网络错误', async () => {
      ...
      expect(monitoringService.recordError).toHaveBeenCalledWith(networkError, expect.any(Object));
    });
  });
});
