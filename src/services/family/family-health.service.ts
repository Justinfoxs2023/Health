...
export class FamilyHealthService {
  ...
  // 添加家庭成员
  async addFamilyMember(userId: string, memberInfo: Partial<FamilyMember>): Promise<void> {
    // 发送邀请
  ...
  async handleHealthAlert(alert: FamilyHealthAlert): Promise<void> {
    // 确定告警接收者
    const recipients = await this.determineAlertRecipients(alert.familyId, alert.priority);
