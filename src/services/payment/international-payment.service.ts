...
export class InternationalPaymentService {
  ...
  async processInternationalPayment(
    ...
    // 确定支付提供商
    const provider = await this.determinePaymentProvider(
      paymentData.method,
      paymentData.region
    );
  ...
  // 处理退款
  async processInternationalRefund(
    refundData: InternationalRefundRequest
  ): Promise<RefundResult> {
    // 验证退款请求
    await this.validateRefundRequest(refundData);
    
    // 确定原支付提供商
    const provider = await this.getOriginalPaymentProvider(
      refundData.paymentId
    );
    
    // 创建退款请求
    const refundRequest = await this.createRefundRequest(
      provider,
      refundData
    );
  ...
  async performComplianceCheck(
    ...
    return await this.complianceService.checkInternationalPayment(
      paymentData
    );
  }
} 