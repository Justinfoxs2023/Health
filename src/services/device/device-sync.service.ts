...
export class DeviceSyncService {
  ...
  // 注册新设备
  async registerDevice(userId: string, deviceInfo: Partial<IHealthDevice>): Promise<IHealthDevice> {
    // 验证设备信息
    await this.validateDevice(deviceInfo);

    // 注册设备
    const device = await this.deviceRegistry.registerDevice(userId, deviceInfo);
  ...
  // 同步设备数据
  async syncDeviceData(deviceId: string): Promise<IHealthRecord[]> {
  ...
  // 数据异常检测
  private async detectAnomalies(records: IHealthRecord[]): Promise<void> {
  ...
  // 处理异常数据
  private async handleAbnormalRecord(record: IHealthRecord): Promise<void> {
    // 发送警告通知
  ...
  // 同步到家庭成员
  private async syncToFamilyMembers(userId: string, records: IHealthRecord[]): Promise<void> {
    const family = await this.familyService.getFamilyByMemberId(userId);

    if (!family) return;

    const healthManager = await this.familyService.getFamilyHealthManager(family.id);

    if (healthManager) {
      await this.healthDataService.shareHealthRecords(records, healthManager.id, 'family_sync');
    }
  }
  ...
  async generateHealthReport(
    ...
    const records = await this.healthDataService.getHealthRecords(userId, timeRange);
