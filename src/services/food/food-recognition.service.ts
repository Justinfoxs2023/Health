import { IFood, IFoodAnalysis } from '../../types/food';
...
export class FoodRecognitionService {
  ...
  // 识别食品图片
  async recognizeFood(imageData: Buffer): Promise<IFood[]> {
    try {
      // 1. 预处理图片
  ...
  // 分析营养成分
  async analyzeNutrition(food: IFood, portion: number): Promise<IFoodAnalysis> {
    try {
      // 1. 计算营养值
