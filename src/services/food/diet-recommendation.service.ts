import { IFood, INutritionInfo } from '../../types/food';
...
export class DietRecommendationService {
  ...
  // 生成饮食计划
  async generateDietPlan(userProfile: UserProfile, preferences: any): Promise<DietPlan> {
    try {
      // 1. 分析用户需求
  ...
  // 分析饮食习惯
  async analyzeDietaryHabits(mealRecords: MealRecord[]): Promise<DietaryAnalysis> {
    try {
      // 1. 分析营养摄入
