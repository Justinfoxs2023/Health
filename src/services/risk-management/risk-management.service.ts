...
import { HealthRisk, RiskAssessment, RiskAlertConfig } from './types';
import * as tf from '@tensorflow/tfjs-node';

@Injectable()
export class RiskManagementService implements OnModuleInit {
  private riskModel: tf.LayersModel;
  private readonly alertConfig: RiskAlertConfig;
  ...
  // 更新风险监测配置
  async updateRiskMonitoring(userId: string, config: Partial<RiskAlertConfig>): Promise<void> {
    try {
  ...
  private async initializeRiskModel() {
    try {
      this.riskModel = await tf.loadLayersModel(
        this.config.get('RISK_MODEL_PATH')
      );
    } catch (error) {
  ...
  private async predictRisks(data: tf.Tensor): Promise<HealthRisk[]> {
  ...
  private async checkAndSendAlerts(userId: string, assessment: RiskAssessment) {
  ...
  private async sendRiskAlerts(userId: string, risks: HealthRisk[]) {
  ...
  private calculateOverallRiskScore(risks: HealthRisk[]): number {
  ...
  private loadAlertConfig(): RiskAlertConfig {
    return {
      thresholds: {
