...
export class FundUsageService {
  ...
  // 处理资金使用请求
  async processFundUsage(accountId: string, request: FundUsageRequest): Promise<ITransaction> {
    // 验证使用规则
  ...
  // 处理提现请求
  async processWithdrawal(accountId: string, request: WithdrawalRequest): Promise<ITransaction> {
    // 验证提现限制
    ...
    // 创建提现交易
    const transaction = await this.createWithdrawalTransaction(accountId, request);
  ...
  async processTransfer(
    ...
    // 创建转账交易
    const transaction = await this.createTransferTransaction(fromAccountId, toAccountId, request);
  ...
  // 生成使用报告
  async generateUsageReport(accountId: string, period: DateRange): Promise<FundUsageReport> {
