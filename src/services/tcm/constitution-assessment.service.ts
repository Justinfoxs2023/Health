...
export class ConstitutionAssessmentService {
  ...
  async performAssessment(
    ...
    // 生成体质报告
    return await this.generateAssessmentReport(
      userId,
      constitutionFeatures
    );
  }
  ...
  async generateHealthSuggestions(
    ...
    // 饮食建议
    suggestions.push(
      await this.getDietarySuggestions(constitution, season)
    );
    
    // 起居建议
    suggestions.push(
      await this.getLifestyleSuggestions(constitution, season)
    );
    
    // 运动建议
    suggestions.push(
      await this.getExerciseSuggestions(constitution, season)
    );
    
    // 情志调养
    suggestions.push(
      await this.getEmotionalSuggestions(constitution)
    );
  ...
  async trackConstitutionChanges(
    ...
    const assessments = await this.getHistoricalAssessments(
      userId,
      period
    );
  ...
  async generateSeasonalPlan(
    ...
    return {
      dietary: await this.generateSeasonalDiet(
        constitution,
        seasonalFactors
      ),
      lifestyle: await this.generateSeasonalLifestyle(
        constitution,
        seasonalFactors
      ),
      exercises: await this.generateSeasonalExercises(
        constitution,
        seasonalFactors
      ),
      preventions: await this.generatePreventiveMeasures(
        constitution,
        seasonalFactors
      )
    };
