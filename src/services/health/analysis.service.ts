...
t class HealthAnalysisService {
  ...
  // 分析健康数据
  async analyzeHealthData(userId: string, period: string): Promise<IHealthAnalysis> {
    try {
      // 1. 收集健康数据
  ...
  // 添加类型定义
  private async parseInsights(content: string): Promise<IHealthInsight[]> {
    try {
      return JSON.parse(content) as IHealthInsight[];
    } catch (error) {
