...
expo
rt class ComprehensiveAnalysisService {
  ...
  // 生成综合健康报告
  async generateHealthReport(userId: string, period: string): Promise<IHealthReport> {
    try {
      // 1. 收集所有健康数据
  ...
  // 分析健康趋势
  private async analyzeHealthTrends(healthData: any, period: string): Promise<TrendAnalysis[]> {
    try {
      // 1. 提取关键指标
