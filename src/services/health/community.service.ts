...
export class HealthCommunityService {
  ...
  // 寻找健康伙伴
  async findHealthPartners(userId: string, criteria: PartnerCriteria): Promise<PartnerMatch[]> {
    try {
      // 1. 获取用户画像
      ...
      // 3. 计算兼容性
      const compatibilityScores = await this.calculateCompatibility(userProfile, matches);
  ...
  // 创建健康挑战
  async createHealthChallenge(challenge: ChallengeConfig): Promise<HealthChallenge> {
    try {
      // 1. 验证挑战配置
