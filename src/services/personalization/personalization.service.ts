...
export class PersonalizationService {
  ...
  // 生成个性化运动计划
  async createExercisePlan(userId: string): Promise<ExercisePlan> {
    try {
      // 1. 获取用户健康风险评估
  ...
  // 生成个性化饮食方案
  async createDietPlan(userId: string): Promise<DietPlan> {
    try {
      // 1. 获取用户健康风险评估
      const riskAssessment = await this.riskService.performRiskAssessment(userId);
  ...
  async updatePlanProgress(
    planId: string,
    progress: Partial<ExercisePlan | DietPlan>
  ): Promise<void> {
    try {
  ...
  private buildExercisePlan(userId: string, recommendations: any): ExercisePlan {
    // 实现运动计划构建逻辑
  }

  private buildDietPlan(userId: string, recommendations: any): DietPlan {
    // 实现饮食��划构建逻辑
  }

  private shouldAdjustPlan(plan: PersonalizedPlan): boolean {
    // 实现计划调整判断逻辑
  }

  private async adjustPlan(plan: PersonalizedPlan): Promise<void> {
    // 实现计划调整逻辑
  }

  private buildProgressReport(plan: PersonalizedPlan) {
    // 实现进度报告生成逻辑
  }
} 