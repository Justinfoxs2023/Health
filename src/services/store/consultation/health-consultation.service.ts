import { IHealthProductInquiry } from '../types';
...
export class HealthConsultationService {
  ...
  // 满意度评价
  async rateProfessionalResponse(
    inquiryId: string,
    rating: number,
    feedback?: string,
  ): Promise<void> {
  ...
  private async saveInquiry(params: any): Promise<IHealthProductInquiry> {
    // 实现保存逻辑
    return null;
  }

  private async assignProfessional(inquiry: IHealthProductInquiry): Promise<void> {
    // 实现分配逻辑
  }
}
