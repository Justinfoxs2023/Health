...
interface ISocialShareProps {
  /** config 的描述 */
    config: OmitSocialShareConfig, /** platform 的描述 */
    /** platform 的描述 */
    platform;
  /** platforms 的描述 */
...
export const SocialShare: React.FC<ISocialShareProps> = ({
  config,
      ...
      switch (platform) {
        ...
        case 'weibo':
          const weiboUrl = `http://service.weibo.com/share/share.php?url=${encodeURIComponent(
            ...
            config.image ? `&pic=${encodeURIComponent(config.image)}` : ''
          }`;
        ...
        case 'qq':
          const qqUrl = `http://connect.qq.com/widget/shareqq/index.html?url=${encodeURIComponent(
            config.url,
          )}&title=${encodeURIComponent(config.title)}&desc=${encodeURIComponent(
            config.description || '',
          )}&pics=${encodeURIComponent(config.image || '')}`;
