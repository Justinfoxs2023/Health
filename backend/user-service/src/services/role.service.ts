...
export class RoleService {
  ...
  private roleConfig: RoleConfigMap;

  constructor() {
    this.redis = new RedisClient();
    this.logger = new Logger('RoleService');
    this.roleConfig = RoleConfig as RoleConfigMap;
  }

  /**
   * 检查用户角色
   */
  public async checkUserRole(userId: string, roleCode: RoleCode): Promise<boolean> {
    try {
      // 检查缓存
      ...
      const parsedRoles = JSON.parse(userRoles) as RoleCode[];
   ...
   * 分配用户角色
   */
  public async assignUserRole(userId: string, roleCode: RoleCode): Promise<void> {
    try {
      // 验证角色是否存在
   ...
   * 获取角色权限列表
   */
  public async getRolePermissions(roleCode: RoleCode): Promise<string[]> {
    try {
