...
export const HealthDataValidation = Joi.object({
  type: Joi.string().valid('vital', 'activity', 'sleep', 'nutrition').required(),
  ...
  metrics: Joi.object({
    ...
    bloodSugar: Joi.number().min(2).max(30),

    weight: Joi.number().min(1).max(500),

    height: Joi.number().min(30).max(300),

    temperature: Joi.number().min(35).max(43),

    oxygenSaturation: Joi.number().min(50).max(100),
