...
export class MultiTaskLearningService {
  ...
  // 添加任务
  async addTask(taskConfig: TaskConfig): Promise<void> {
  ...
  private async buildModel(): Promise<tf.LayersModel> {
    ...
    const losses = Array.from(this.tasks.values()).reduce((acc, task) => {
      acc[`output_${task.id}`] = task.type === 'classification' 
  ...
  // 训练模型
  async train(
    data: {[taskId: string]: { x: tf.Tensor; y: tf.Tensor }}
  ): Promise<tf.History> {
    if (!this.model) {
    ...
    const ys = Object.fromEntries(
      Array.from(this.tasks.keys()).map(taskId => 
        [`output_${taskId}`, data[taskId].y]
      )
    );
  ...
  async predict(
    input: tf.Tensor
  ): Promise<{[taskId: string]: tf.Tensor}> {
    if (!this.model) {
  ...
  async evaluate(
    ...
    return Object.fromEntries(
      Array.from(this.tasks.keys()).map((taskId, index) => 
        [taskId, evaluation[index].dataSync()[0]]
      )
    );
  }
